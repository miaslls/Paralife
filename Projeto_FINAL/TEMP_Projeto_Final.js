...
// ----- executa a atividade de BANHEIRO escolhida FIXME:
...
console.log("activityList_toilet[0]", activityList_toilet[0]); // FIXME:
console.log(".timeToComplete", activityList_toilet[0].timeToComplete);
...
`);
    ...
    case 3: {
      ...
      for (let hygieneActivity of activityList_hygiene) {
        console.log(`[${hygieneActivity.index}] ${hygieneActivity.title.toUpperCase()}`);
      }
...
`);
    ...
    case 3: {
      ...
      for (let toiletActivity of activityList_toilet) {
        console.log(`[${toiletActivity.index}] ${toiletActivity.title.toUpperCase()}`);
      }
      ...
      console.log(toiletActivityChoiceIndex); // FIXME:
      chosenToiletActivity = activityList_toilet[0];
  ...
  atributos: \t+${chosenToiletActivity.needsModification.toilet} 🚽 | -${chosenToiletActivity.needsModification.hygiene} 🧼
`);
...
switch (mainMenuChoice) {
  ...
  case 4: {
    doToiletActivity(chosenToiletActivity); // executa a atividade BANHEIRO FIXME:
    break;
  }
  case 5: {
